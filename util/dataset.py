...
class SoccerNetClips(Dataset):
    def __init__(self, path, features="baidu_soccer_embeddings.npy", split=["train"], version=2,
        ...
        downloader.downloadGames(files=[self.labels, f"1_{self.features}", f"2_{self.features}"], split=split,
                                 verbose=False, randomized=True)
    ...
    def __init__(self, path, features="baidu_soccer_embeddings.npy", split=["test"], version=2,
        ...
        for s in split:
            if s == "challenge":
                ...
                downloader.downloadGames(files=[self.labels, f"1_{self.features}", f"2_{self.features}"], split=[s],
                                         verbose=False, randomized=True)
